...
func (db *MySQLDatabase) DeleteDoctorByID(ctx context.Context, id int) (int64, error) {
	...
	log.Printf("Executing delete query for doctor with ID %d...", id)

	// Execute the SQL statement
	result, err := db.ExecContext(ctx, query, id)
	if err != nil {
		log.Printf("Error deleting doctor with ID %d: %v", id, err)
		return 0, err
	}
	...
	if err != nil {
		log.Printf("Error fetching rows affected for delete query of doctor with ID %d: %v", id, err)
		return 0, err
	}
