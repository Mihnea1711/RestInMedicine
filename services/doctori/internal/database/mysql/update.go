...
func (db *MySQLDatabase) UpdateDoctorByID(ctx context.Context, doctor *models.Doctor) (int64, error) {
	...
	log.Printf("Executing update query for doctor with ID %d...", doctor.IDDoctor)
	...
	if err != nil {
		log.Printf("Error updating doctor with ID %d: %v", doctor.IDDoctor, err)
		return 0, err
	}
	...
	if err != nil {
		log.Printf("Error fetching rows affected for update query of doctor with ID %d: %v", doctor.IDDoctor, err)
		return 0, err
	}

	if rowsAffected == 0 {
		log.Printf("No doctor found with ID %d to update.", doctor.IDDoctor)
	} else {
		log.Printf("Successfully updated doctor with ID %d.", doctor.IDDoctor)
	}
