...
func (r *RedisRateLimiter) Limit(next http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if err != nil {
			log.Printf("[APPOINTMENT_LIMITER] Error splitting remote addr %s", err) // Logging the error
		...
		if err != nil {
			log.Printf("[APPOINTMENT_LIMITER] Error incrementing rate limit key %s: %v", key, err) // Logging the error
			http.Error(w, "Internal Server Error", http.StatusInternalServerError)
			return
		}
		if val == 1 {
