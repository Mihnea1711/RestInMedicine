...
func (rc *RedisClient) RemoveUserFromBlacklistInRedis(ctx context.Context, user models.User) (int64, error) {
	// Generate a key to uniquely identify the user in the blacklist
	blacklistKey := fmt.Sprintf("blacklist:%d", user.IDUser)
	...
	if err != nil {
		log.Printf("[IDM] Error removing user (ID %d) from Redis blacklist: %v", user.IDUser, err)
		return 0, err
	}

	log.Printf("[IDM] Removed user (ID %d) from Redis blacklist, rows affected: %d", user.IDUser, rowsAffected)
